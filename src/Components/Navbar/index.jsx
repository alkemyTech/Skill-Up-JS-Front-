...
export const Navbar = () => {
  ...
  return (
    <>
      ...
      {sidebarOpen && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} className='flex flex-col items-center space-y-3 text-white fixed w-[200px] h-[250px] left-[50%] top-[5%] ml-[-100px] bg-black rounded-xl'>
          <Link onClick={() => setSidebarOpen(false)} to='/transactions'>Transactions</Link>
          <Link onClick={() => setSidebarOpen(false)} to='/deposit'>Deposit</Link>
          <Link onClick={() => setSidebarOpen(false)} to='/pay'>Pay</Link>
          <Link onClick={() => setSidebarOpen(false)} to='/balance'>Balance</Link>
          <Link onClick={() => setSidebarOpen(false)} to='/send'>Send Money</Link>
          <Link onClick={() => setSidebarOpen(false)} to='/transactions'>Profile</Link>
          <Link onClick={() => setSidebarOpen(false)} to='/transactions'>Logout</Link>
        </motion.div>
      )}
